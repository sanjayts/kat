...
const PRG: &str = "kat";
const MOVIES_CSV: &str = "tests/inputs/movies1.csv";
const MOVIES_TSV: &str = "tests/inputs/movies1.tsv";
const BOOKS_TSV: &str = "tests/inputs/books.tsv";
const BOOKS_CSV: &str = "tests/inputs/books.csv";
...
fn skips_bad_file() -> TestResult {
    ...
    Command::cargo_bin(PRG)?
        .args(&["-f", "1", MOVIES_CSV, &bad, MOVIES_TSV])
        .assert()
        .success()
...
fn dies_not_enough_args() -> TestResult {
    dies(&[MOVIES_CSV], "Must have --fields, --bytes, or --chars")
}
...
fn dies_empty_delimiter() -> TestResult {
    dies(&[MOVIES_CSV, "-f", "1", "-d", ""], "kat: bad delimiter")
}
...
fn tsv_f1() -> TestResult {
    run(
        &[MOVIES_TSV, "-f", "1"],
        "tests/expected/movies1.tsv.f1.out",
    )
}
...
fn tsv_f2() -> TestResult {
    run(
        &[MOVIES_TSV, "-f", "2"],
        "tests/expected/movies1.tsv.f2.out",
    )
}
...
fn tsv_f3() -> TestResult {
    run(
        &[MOVIES_TSV, "-f", "3"],
        "tests/expected/movies1.tsv.f3.out",
    )
}
...
fn tsv_f1_2() -> TestResult {
    run(
        &[MOVIES_TSV, "-f", "1-2"],
        "tests/expected/movies1.tsv.f1-2.out",
    )
}
...
fn tsv_f2_3() -> TestResult {
    run(
        &[MOVIES_TSV, "-f", "2-3"],
        "tests/expected/movies1.tsv.f2-3.out",
    )
}
...
fn tsv_f1_3() -> TestResult {
    run(
        &[MOVIES_TSV, "-f", "1-3"],
        "tests/expected/movies1.tsv.f1-3.out",
    )
}
...
fn tsv_b1() -> TestResult {
    run(
        &[MOVIES_TSV, "-b", "1"],
        "tests/expected/movies1.tsv.b1.out",
    )
}
...
fn tsv_b2() -> TestResult {
    run(
        &[MOVIES_TSV, "-b", "2"],
        "tests/expected/movies1.tsv.b2.out",
    )
}
...
fn tsv_b8() -> TestResult {
    run_lossy(
        &[MOVIES_TSV, "-b", "8"],
        "tests/expected/movies1.tsv.b8.out",
    )
}
...
fn tsv_b1_2() -> TestResult {
    run(
        &[MOVIES_TSV, "-b", "1-2"],
        "tests/expected/movies1.tsv.b1-2.out",
    )
}
...
fn tsv_b2_3() -> TestResult {
    run(
        &[MOVIES_TSV, "-b", "2-3"],
        "tests/expected/movies1.tsv.b2-3.out",
    )
}
...
fn tsv_b1_8() -> TestResult {
    run_lossy(
        &[MOVIES_TSV, "-b", "1-8"],
        "tests/expected/movies1.tsv.b1-8.out",
    )
}
...
fn tsv_c1() -> TestResult {
    run(
        &[MOVIES_TSV, "-c", "1"],
        "tests/expected/movies1.tsv.c1.out",
    )
}
...
fn tsv_c2() -> TestResult {
    run(
        &[MOVIES_TSV, "-c", "2"],
        "tests/expected/movies1.tsv.c2.out",
    )
}
...
fn tsv_c8() -> TestResult {
    run(
        &[MOVIES_TSV, "-c", "8"],
        "tests/expected/movies1.tsv.c8.out",
    )
}
...
fn tsv_c1_2() -> TestResult {
    run(
        &[MOVIES_TSV, "-c", "1-2"],
        "tests/expected/movies1.tsv.c1-2.out",
    )
}
...
fn tsv_c2_3() -> TestResult {
    run(
        &[MOVIES_TSV, "-c", "2-3"],
        "tests/expected/movies1.tsv.c2-3.out",
    )
}
...
fn tsv_c1_8() -> TestResult {
    run(
        &[MOVIES_TSV, "-c", "1-8"],
        "tests/expected/movies1.tsv.c1-8.out",
    )
}
...
fn repeated_value() -> TestResult {
    run(&[BOOKS_TSV, "-c", "1,1"], "tests/expected/books.c1,1.out")
}

#[test]
fn books_quoted_when_delim() -> TestResult {
    run(
        &[BOOKS_CSV, "-f", "2,3", "-d", ","],
        "tests/expected/books_csv.f2,3.out",
    )
}
